...
The cell type classification will be performed on the RNA expression only, since most information is available for marker genes. To find genes that might be most useful for assigning clusters a Differential Expression analysis is performed.

```{r}
DefaultAssay(mbrain) = "SCT" # run marker finding on normalized RNA data
mbrain.markers <- FindAllMarkers(mbrain, only.pos = T) # DE
```
...
```{r}
sctype.markers <- read_csv("sc-type_brain.csv") %>% 
...
according to the UMAP c5 is likely the precursor and c9 + c13 are likely differentiated.
...
```{r}
sctype.markers %>% distinct(celltype) %>% dplyr::filter(str_detect(celltype, "neuron"))
```
### Oligodendrocytes

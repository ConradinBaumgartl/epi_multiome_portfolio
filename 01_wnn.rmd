...
```{r}
VlnPlot(mbrain, features = c("nCount_ATAC", "nCount_RNA"), pt.size = 0, log=T)
```
...
p2 <- DimPlot(mbrain, reduction = "umap.atac", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("ATAC")
p3 <- DimPlot(mbrain, reduction = "wnn.umap", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("WNN")
